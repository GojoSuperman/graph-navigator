...
const TERM_NOISE = new Set([
  ...
  /**
   * **질문의 틀**은 조건이 아니다 — 실측으로 잡았다.
   *
   * "역대 한국 영화 흥행 상위권 작품 중, **저승 세계**를 소재로 한 **판타지**
   * 영화는?" 의 조건 낱말이 `역대·한국·흥행·상위권·저승` 으로 잡혔다.
   * 다섯 칸 중 넷을 틀이 차지하고 **정작 '판타지' 는 잘려 나갔다.**
   * 그러고는 "역대·흥행·상위권 조건을 충족하지 않습니다" 라고 답했다 —
   * 줄거리에 '흥행' 이라고 적힌 영화는 세상에 없다.
   *
   * 수상 기록에서 이미 한 번 고친 것과 **같은 결함**이다 (→ pickAwards).
   * 자르는 것이 아니라 **정렬 없이 앞에서 자르는 것**이 틀렸다.
   *
   * 아래 낱말은 평가셋 152문항에서 **두 문항 이상에 반복해서 나온 틀**이다.
   * 빈도 — 무엇인가요(19) 국내(12) 언제인가요(9) 개봉일(7) 한국(6) 흥행(5)
   * 박스오피스(3) 기록한(3) 누구인가요(2) 제목(2) 역대(2) 상위권(2) 개봉(2) 연도(2).
   * 반대로 기생충·독일·기자·가족·아가씨 처럼 **내용을 가리키는 것은 남겨 뒀다.**
   */
  "무엇인가요", "누구인가요", "언제인가요", "어디인가요", "제목",
  "국내", "개봉", "개봉일", "연도",
  "한국", "역대", "흥행", "상위권", "박스오피스", "기록한",
]);
...
function termCheck(r: AskResult): string[] {
  ...
  const scored = r.evidence.map((e) => {
    // 장르도 센다 — '판타지' · '코미디' 는 줄거리에 안 적히고 **장르 칸에** 있다.
    // 근거 블록에는 장르가 보이는데 여기서 안 세면, 모델이 보는 것과 코드가
    // 세는 것이 어긋난다.
    const text = `${e.title} ${e.genres.join(" ")} ${e.overview ?? ""}`;

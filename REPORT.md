...
**① 개체 이름이 없는 질문을 못 받습니다** (알고 있는 결함)

```
질문   폭군 정치를 한 왕은 누구야?
경유   followup → route → refuse
```

`route` 가 질문에서 노드 이름을 못 찾으면 **바로 거절**합니다. `seeds` 노드에 BM25
폴백을 두었는데 **route 가 먼저 끊어서 도달하지 못하는 죽은 코드**입니다. 게다가 그
BM25 는 **노드 이름만 색인**하고 본문을 색인하지 않습니다 — 평가의 BM25 대조군은
본문을 색인하는데, **같은 이름의 다른 물건**이었습니다.
→ `route` 를 완화하고 BM25 색인에 본문을 넣어야 합니다. **거절 동작이 바뀌므로
평가를 다시 돌려야 합니다.**

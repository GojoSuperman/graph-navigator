...
| 같은 관계 중복 | `from·to·kind` 중복 제거, **근거는 여러 개 보관** | 2,197 → 1,619 |

```
노드 1,221 → 1,134   (87개 별칭으로 병합)
...
```
  ...
  2홉        26/27 ( 96%)   23/27 ( 85%)
  3홉         4/7  ( 57%)    5/7  ( 71%)
  ──
  전체       40/44 ( 91%)   38/44 ( 86%)
  거절        8/8  (100%)      —
```
...
```
  1홉 80%  ·  2홉 56%  ·  3홉 28%  ·  전체 57%
```

기대 홉의 43% 가 실제 탄 경로에 없습니다. **예산을 늘려도 안 오르므로 탐색 문제가
아닙니다** — 기대 경로의 표기가 그래프와 다르거나, 탐색이 다른 길로 도달한 것입니다.

> **씨앗 보강이 경로 재현율을 10%p 깎았습니다** (6장 ① 참조). 이름이 걸린 노드가
> 막다른 곳이면 BM25 로 씨앗을 더하는데, **출발점이 달라지니 기대 경로와 다른 길로
> 갑니다.** 컨텍스트 재현율은 89→91% 로 올랐고 경로 재현율은 67→57% 로 내렸습니다.
>
> ```
>              전      후
>   컨텍스트    89%     91%     ← 답에 더 잘 닿는다
>   경로        67%     57%     ← 기대한 길로는 덜 간다
>   답변        61%     61%
>   거절       8/8     8/8
> ```
>
> **답에 닿는 것과 기대한 길로 가는 것이 다른 목표라는 것**이 이 교환에서 드러났습니다.
> 루브릭 2가 "실제로 탄 경로를 제시하는가" 를 묻는 이상 경로 재현율도 지켜야 할
> 숫자인데, 골든셋의 기대 경로는 *"질문에 나온 개체에서 출발한다"* 를 전제로 썼습니다.
> **전제가 바뀌었으면 기대 경로도 다시 써야 맞지만, 그것은 채점표를 고치는 일이라
> 하지 않았습니다.** 숫자를 그대로 싣습니다.
...
직접 셌고, **한계도 화면에 적었습니다** (스키마의 근대 편향 · 문서를 가진 개체 112/1,134).
...
| LLM 추출은 `temperature: 0` 이면 재현된다 | **39% 가 달라진다.** 같은 문서를 다시 읽어도 다르게 뽑는다 |
...
**② 스키마의 인물 관계 결함 — 실험했고, 그 실험이 더 중요한 것을 알려 줬습니다**

관계 6종이 전부 조직·사건 중심이라 **인물 간 관계는 `STUDIED_UNDER` 하나뿐**입니다.
그래서 *"자식을 죽인 임금은 누구야?"* 에 답하지 못했습니다 — 《영조》 문서에 사도세자
서술이 **20문장 넘게** 있는데도 `사도세자` 가 **노드로조차 안 생겼습니다.** 담을 관계가
없으니 추출기가 뽑을 수 없었습니다.

코퍼스가 얼마나 버려지고 있었는지 쟀습니다.

```
혈연 서술    2,193회 · 116/140 문서
대립·정적    1,030회 · 108/140 문서
혼맥           565회 ·  94/140 문서
처형·죽임      398회 ·  80/140 문서
```

**조선 인물사의 핵심이 대부분 여기 있었습니다.**

#### 실험 — `FAMILY_OF`·`OPPOSED` 를 넣고 전량 재추출

측정할 것을 먼저 만들었습니다 — **인물 간 관계 12문항**
(`data/history/probe-person.json`). 답이 코퍼스 본문에 실제로 서술된 것만 골랐고,
골든셋과 **따로** 뒀습니다. 섞으면 "스키마가 못 담아서 틀린 것" 과 "탐색이 못 찾아서
틀린 것" 이 한 숫자에 뭉칩니다.
...
```
  ...
  혼맥         0/1      1/1
  ──
  전체     5/12 (42%)  10/12 (83%)     엣지 2,256 → 4,114 (+82%) · $1.90
```

`영조 ─혈연─▶ 사도세자` 와 `영조 ─대립─▶ 사도세자` 가 **둘 다** 잡혔습니다.
뒤주 문장은 부자 관계이면서 죽임이라, 프롬프트에 *"한 문장이 두 관계를 말하면 둘 다
낸다"* 를 넣어야 했습니다.

#### 그런데 되돌렸습니다 — **LLM 추출이 39% 비결정적입니다**

```
v2 고유 삼중항 1,700개 중
  v3 에도 있음   1,023 (60%)
  사라짐           677 (39%)   ← 관계 추가와 무관하게, 같은 문서를 다시 읽어도 다르게 뽑는다

→ 골든셋 52문항 중 14건이 "근거 문장이 그래프에 없다" 로 깨졌다
```

**`temperature: 0` 인데도 재현되지 않습니다.** 관계 2종을 넣어 프롬프트가 길어진 영향도
있겠지만, 근본적으로 **추출은 결정적 연산이 아닙니다.**

그래서 6종으로 되돌렸습니다. 골든셋·홀드아웃·6단계 스윕 값이 전부 v2 그래프에 묶여
있고, **그 골든셋을 v3 기준으로 다시 쓰면 과적합 측정이 무의미해집니다** — 홀드아웃을
다시 보고 고치는 일이 되기 때문입니다. 깨진 14문항을 빼는 것도 **틀린 문항을 지워
점수를 올리는 꼴**입니다.

실험 결과와 v3 추출물(`edges.raw.v3-family.jsonl`)은 **증거로 커밋해 두었습니다.**

> **이 실험이 알려 준 가장 중요한 것.** 추출 결과를 산출물로 커밋해 둔 것이 올바른
> 선택이었습니다. 안 그랬으면 이 39% 를 영영 몰랐고, **재현할 수 없는 그래프 위에서
> 측정하고 있었을 것입니다.** 같은 코드를 돌려도 같은 그래프가 안 나오는데, 커밋된
> 그래프가 없으면 REPORT 의 모든 숫자가 재현 불가능해집니다.

> **남은 어휘 불일치 문제.** `FAMILY_OF` 를 넣어도 *"자식을 죽인 임금은?"* 은 여전히
> 거절합니다. 코퍼스는 `세자`·`영조`·`뒤주` 로 쓰는데 질문은 `자식`·`임금` 으로
> 묻습니다. BM25 최고점이 9.6 이라 하한을 낮춰도 1위가 엉뚱한 `정조` 입니다 —
> **어휘를 잇는 층(동의어·임베딩)이 없다는 뜻이고, 관계를 더 넣어 해결되는 문제가
> 아닙니다.** 반면 코퍼스 어휘로 물으면 답합니다: *"뒤주에 갇혀 죽은 세자는?"* →
> 사도세자 28.8점.
